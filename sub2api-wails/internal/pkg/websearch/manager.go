...
import (
	...
	"sub2api-wails/internal/pkg/proxyutil"
...
func NewManager(configs []ProviderConfig, redisClient *redismem.RedisStub) *Manager {
...
func (m *Manager) tryReserveQuota(ctx context.Context, cfg ProviderConfig) (bool, bool) {
	...
	newVal, err := m.quotaIncr(ctx, key, int64(ttlSec))
	if err != nil {
...
func (m *Manager) GetUsage(ctx context.Context, providerType string) (int64, error) {
	...
	val, err := m.redisGetInt64(ctx, key)
	if err != nil {
		return 0, nil
	}
	return val, err
...
func (m *Manager) quotaIncr(ctx context.Context, key string, ttlSec int64) (int64, error) {
	...
	if newVal == 1 {
		m.redis.Expire(ctx, key, time.Duration(ttlSec)*time.Second)
	}
	return newVal, nil
}
