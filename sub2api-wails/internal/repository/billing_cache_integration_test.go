	...
	tests := []struct {
	...
	}{
		{
			...
			fn: func(ctx context.Context, rdb *redis.Client, cache service.BillingCache) {
				...
				require.ErrorIs(s.T(), err, redismem.Nil, "expected redismem.Nil for missing balance key")
			},
		},
		{
			...
			fn: func(ctx context.Context, rdb *redis.Client, cache service.BillingCache) {
				...
				require.ErrorIs(s.T(), err, redismem.Nil, "expected missing key after deduct on non-existent")
			},
		},
		{
		...
		},
		{
			...
			fn: func(ctx context.Context, rdb *redis.Client, cache service.BillingCache) {
				...
				require.ErrorIs(s.T(), err, redismem.Nil, "expected redismem.Nil after invalidate")
			},
		},
		{
	...
	tests := []struct {
	...
	}{
		{
			...
			fn: func(ctx context.Context, rdb *redis.Client, cache service.BillingCache) {
				...
				require.ErrorIs(s.T(), err, redismem.Nil, "expected redismem.Nil for missing subscription key")
			},
		},
		{
		...
		},
		{
			...
			fn: func(ctx context.Context, rdb *redis.Client, cache service.BillingCache) {
				...
				require.NotErrorIs(s.T(), err, redismem.Nil, "expected parsing error, not redismem.Nil")
...
// Redis 真实错误应传播，key 不存在（redismem.Nil）应返回 nil。
func (s *BillingCacheSuite) TestDeductUserBalance_ErrorPropagation() {
	tests := []struct {
	...
	}{
		{
			...
			fn: func(ctx context.Context, cache service.BillingCache) {
				// key 不存在时，Lua 脚本返回 0（redismem.Nil），应返回 nil 而非错误
