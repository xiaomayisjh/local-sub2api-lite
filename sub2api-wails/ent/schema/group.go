...
import (
	"sub2api-wails/ent/schema/mixins"
...
func (Group) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Float("rate_multiplier").
			SchemaType(map[string]string{dialect.SQLite: "decimal(10,4)"}).
			Default(1.0),
		...
		field.Float("daily_limit_usd").
			Optional().
			Nillable().
			SchemaType(map[string]string{dialect.SQLite: "decimal(20,8)"}),
		...
		field.JSON("model_routing", map[string][]int64{}).
			Optional().
			SchemaType(map[string]string{dialect.SQLite: "jsonb"}).
