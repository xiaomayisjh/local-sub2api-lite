export default {
  ...
  // Setup Wizard
  setup: {
    title: 'ANT-Sub2API-Local Setup',
    description: 'Configure your ANT-Sub2API-Local instance',
    database: {
  ...
  admin: {
    ...
    accounts: {
      ...
      batchTestDescription: 'Test {count} selected accounts',
      batchTestAutoMode: 'Will auto-test {count} selected accounts: each account fetches its own model list from upstream and a suitable model is auto-selected for testing. Accounts whose upstream model list cannot be fetched will be marked as failed.',
      batchTestSelectModels: 'Select models to test',
      batchTestNoModels: 'No models available for the selected accounts. Ensure model mappings are configured, or pick different accounts.',
      batchTestModelsHint: '{count} models · {selected} selected',
      batchTestUpstreamOk: '{count} synced from upstream',
      batchTestUpstreamFallback: '{count} fell back to defaults',
      batchTestConcurrency: 'Concurrency (1-20)',
      batchTestStart: 'Start Test',
      batchTestRunning: 'Testing...',
      batchTestCompleted: 'Completed',
      batchTestProgress: 'Progress: {current} / {total}',
      batchTestFailedSummary: '{count} tests failed',
      batchTestSuccessSummary: '{count} tests succeeded',
      batchTestEnableSuccess: 'Enable successful accounts',
      batchTestDisableSuccess: 'Disable successful accounts',
      batchTestMarkSuccessActive: 'Mark successful as active',
      batchTestEnableFailed: 'Enable failed accounts',
      batchTestDisableFailed: 'Disable failed accounts',
      batchTestBulkActionDone: 'Applied · {ok} ok, {fail} failed (out of {total})',
      batchTestBulkActionFailed: 'Bulk action failed: {error}',
      batchTestAttempts: '{count} attempts',
      batchTestFellBack: 'succeeded after falling back to {mode} mode',
      batchTestMarkFailedAsError: 'Mark failed accounts as error',
      batchTestModelSourceFallback: 'known model',
      batchTestModelSourceFallbackHint: 'OAuth accounts often lack a /v1/models endpoint (or the scope forbids it); the test fell back to a known built-in model list for this platform.',
      batchTestUnavailableBadge: 'unavailable',
      batchTestUnavailableHint: 'Temporary upstream/relay issue (no accounts available, gateway 502/503, service busy, or the model is retired with no channel) — not a credential error for this account.',
      batchTestUnavailableSummary: '{count} temporarily unavailable',
      batchTestUnavailableNote: 'These are temporary upstream/relay issues (relay out of accounts, gateway 502/503, retired model with no channel); the account credentials were not disproven. Kept enabled by default — retry later or once the upstream recovers.',
      batchTestKeepUnavailableEnabled: 'Keep enabled (recommended)',
      batchTestDisableUnavailable: 'Disable these accounts anyway',
      findDuplicates: 'Find Duplicates',
      findDuplicatesTitle: 'Find Duplicate Accounts',
      findDuplicatesNone: 'No duplicates found among the {count} selected accounts (compared by credential identity, within the same platform & type).',
      findDuplicatesSummary: 'Found {groups} duplicate group(s), {dupes} redundant account(s) in total. Each group suggests keeping the earliest-created one.',
      findDuplicatesHint: 'Duplicates are matched by credential identity (same platform & type only), not by name. Choose which to keep and which to remove in each group — checked means to delete; nothing is deleted automatically.',
      findDuplicatesGroup: 'Group {index} · {count} accounts',
      findDuplicatesSuggestKeep: 'Suggested keep',
      findDuplicatesCreatedAt: 'created',
      findDuplicatesLastUsed: 'last used',
      findDuplicatesWillKeep: 'Keep',
      findDuplicatesWillDelete: 'Delete',
      findDuplicatesDeleteSelected: 'Delete selected ({count})',
      findDuplicatesDeleteOk: 'Deleted {count} duplicate account(s).',
      findDuplicatesDeletePartial: 'Done: {ok} succeeded, {failed} failed.',
      findDuplicatesAllSelectedWarning: 'At least one group has all accounts checked for deletion. Keep at least one per group to avoid wiping the whole group.',
      findDuplicatesLoadFailed: 'Failed to find duplicate accounts.',
      refresh: {
        errors: {
          REFRESH_TOKEN_REVOKED: 'The refresh token has been revoked or is no longer valid (the account may have been logged out, had its password changed, or expired due to inactivity). Delete the account and re-authorize via OAuth.',
          REFRESH_TOKEN_REUSED: 'The refresh token has already been used (single-use rotation). Re-authorize the account via OAuth to obtain a new one.',
          OAUTH_INVALID_CLIENT: 'OAuth client configuration is invalid. Verify that client_id / client_secret have not been changed upstream.',
          OAUTH_ACCESS_DENIED: 'The upstream OAuth provider denied access. The account may have been disabled or had its scope revoked — re-authorize it.',
          OAUTH_NO_REFRESH_TOKEN: 'This account has no stored refresh_token, so automatic refresh is not possible. Re-add the account via the OAuth flow.',
          OAUTH_MISSING_PROJECT_ID: 'This account is missing project_id, which is required to refresh. Run the full OAuth flow again or fill in project_id manually.',
          OAUTH_UPSTREAM_UNAVAILABLE: 'Could not reach the OAuth service (network drop, connection closed, or timeout). Retry shortly, or check the proxy / outbound network.',
          OAUTH_UPSTREAM_REJECTED: 'The OAuth upstream rejected the refresh request without specifying a reason. Retry shortly; re-authorize if it keeps failing.',
          OPENAI_OAUTH_PROXY_REQUIRED: 'No proxy that can reach OpenAI is configured. Bind a working proxy to this account, then retry.',
          OPENAI_OAUTH_REQUEST_FAILED: 'OpenAI OAuth request failed (typically a proxy or network issue). Retry shortly.',
          OPENAI_OAUTH_TOKEN_EXCHANGE_FAILED: 'OpenAI rejected the authorization-code exchange. Generate a fresh authorization URL and complete the flow again.',
          OPENAI_OAUTH_TOKEN_REFRESH_FAILED: 'OpenAI rejected the token refresh. Verify the refresh token is still valid, or re-authorize the account.',
          OPENAI_OAUTH_CLIENT_INIT_FAILED: 'OpenAI OAuth client failed to initialize. Verify the proxy configuration and retry.',
          OAUTH_REFRESH_FAILED: 'OAuth refresh failed. Retry shortly or re-authorize.'
        }
      },
      selectedCount: '{count} selected',
      ...
      dataImportDedupHint: 'Items duplicating existing accounts are skipped automatically during import (matched by credential identity, within the same platform & type) — no duplicates are created.',
      dataImportSkippedTitle: 'Skipped {count} duplicate account(s)',
      dataImportSuccess: 'Import completed: accounts {account_created}, skipped {account_skipped}, failed {account_failed}',
      ...
      crs: {
        errors: {
          CRS_INVALID_BASE_URL: 'The CRS base URL is invalid. Verify the URL format (must start with https://).',
          CRS_MISSING_CREDENTIALS: 'CRS username and password are required.',
          CRS_AUTH_FAILED: 'CRS login failed. Verify the username / password and that the account is active.',
          CRS_UPSTREAM_UNAVAILABLE: 'Could not reach the CRS service (network drop / timeout / DNS failure). Retry shortly, or check the proxy / outbound network.',
          CRS_EXPORT_FAILED: 'CRS export failed. Verify CRS version is ≥ v1.1.240 and the current user has admin permission.',
          CRS_SYNC_FAILED: 'CRS sync failed. Retry shortly.'
        }
      },
      ...
      bulkActions: {
        ...
        refreshFailureGroupReauth: '{count} need re-authorization',
        refreshFailureGroupTransient: '{count} hit network / upstream transient errors (safe to retry)',
        refreshFailureGroupOther: '{count} other errors'
      },
      bulkEdit: {
    ...
    settings: {
      ...
      site: {
        ...
        siteName: 'Site Name',
        siteNamePlaceholder: 'ANT-Sub2API-Local',
  ...
  onboarding: {
    ...
    admin: {
      welcome: {
        title: '👋 Welcome to ANT-Sub2API-Local',
        description: '<div style="line-height: 1.8;"><p style="margin-bottom: 16px;">ANT-Sub2API-Local is a powerful AI service gateway platform that helps you easily manage and distribute AI services.</p><p style="margin-bottom: 12px;"><b>🎯 Core Features:</b></p><ul style="margin-left: 20px; margin-bottom: 16px;"><li>📦 <b>Group Management</b> - Create service tiers (VIP, Free Trial, etc.)</li><li>🔗 <b>Account Pool</b> - Connect multiple upstream AI service accounts</li><li>🔑 <b>Key Distribution</b> - Generate independent API Keys for users</li><li>💰 <b>Billing Control</b> - Flexible rate and quota management</li></ul><p style="color: #c15f3c; font-weight: 600;">Let\'s complete the initial setup in 3 minutes →</p></div>',
        nextBtn: 'Start Setup 🚀',
        prevBtn: 'Skip'
      },
      groupManage: {
        title: '📦 Step 1: Group Management',
        description: '<div style="line-height: 1.7;"><p style="margin-bottom: 12px;"><b>What is a Group?</b></p><p style="margin-bottom: 12px;">Groups are the core concept of ANT-Sub2API-Local, like a "service package":</p><ul style="margin-left: 20px; margin-bottom: 12px; font-size: 13px;"><li>🎯 Each group can contain multiple upstream accounts</li><li>💰 Each group has independent billing multiplier</li><li>👥 Can be set as public or exclusive</li></ul><p style="margin-top: 12px; padding: 8px 12px; background: #f6e5dc; border-left: 3px solid #c15f3c; border-radius: 4px; font-size: 13px;"><b>💡 Example:</b> You can create "VIP Premium" (high rate) and "Free Trial" (low rate) groups</p><p style="margin-top: 16px; color: #c15f3c; font-weight: 600;">👉 Click "Group Management" on the left sidebar</p></div>'
      },
      createGroup: {
    ...
    user: {
      welcome: {
        title: '👋 Welcome to ANT-Sub2API-Local',
        description: '<div style="line-height: 1.8;"><p style="margin-bottom: 16px;">Hello! Welcome to the ANT-Sub2API-Local AI service platform.</p><p style="margin-bottom: 12px;"><b>🎯 Quick Start:</b></p><ul style="margin-left: 20px; margin-bottom: 16px;"><li>🔑 Create API Key</li><li>📋 Copy key to your application</li><li>🚀 Start using AI services</li></ul><p style="color: #c15f3c; font-weight: 600;">Just 1 minute, let\'s get started →</p></div>',
        nextBtn: 'Start 🚀',
        prevBtn: 'Skip'
      },
      keyManage: {
