export default {
  ...
  // Setup Wizard
  setup: {
    title: 'ANT-Sub2API-Local 安装向导',
    description: '配置您的 ANT-Sub2API-Local 实例',
    database: {
      title: '数据库配置',
  ...
  admin: {
    ...
    accounts: {
      ...
      viewColumns: '列显示',
      batchTest: '批量测试',
      batchTestDescription: '将对选中的 {count} 个账号进行批量测试',
      batchTestAutoMode: '将自动测试选中的 {count} 个账号：为每个账号从上游分别获取模型，挑选一个合适的模型进行测试（无法获取上游模型的账号会标记为失败）。',
      batchTestSelectModels: '选择要测试的模型',
      batchTestNoModels: '所选账号没有可用的模型；请确认账号已配置模型映射，或换其它账号再试。',
      batchTestModelsHint: '共 {count} 个模型，已选 {selected} 个',
      batchTestUpstreamOk: '{count} 个账号已从上游同步',
      batchTestUpstreamFallback: '{count} 个账号回落到默认模型',
      batchTestConcurrency: '并发数（1-20）',
      batchTestStart: '开始测试',
      batchTestRunning: '测试中...',
      batchTestCompleted: '测试完成',
      batchTestProgress: '进度：{current} / {total}',
      batchTestFailedSummary: '{count} 个测试失败',
      batchTestSuccessSummary: '{count} 个测试成功',
      batchTestEnableSuccess: '启用全部成功账号',
      batchTestDisableSuccess: '禁用全部成功账号',
      batchTestMarkSuccessActive: '将成功账号标记为激活',
      batchTestEnableFailed: '启用全部失败账号',
      batchTestDisableFailed: '禁用全部失败账号',
      batchTestBulkActionDone: '已应用 · 成功 {ok}，失败 {fail}（共 {total}）',
      batchTestBulkActionFailed: '批量操作失败：{error}',
      batchTestAttempts: '共尝试 {count} 次',
      batchTestFellBack: '回落到 {mode} 模式后成功',
      batchTestMarkFailedAsError: '将失败账号标记为错误',
      batchTestModelSourceFallback: '已知模型',
      batchTestModelSourceFallbackHint: 'OAuth 账号无 /v1/models 接口或 scope 不允许，已用平台内置已知模型测试。',
      batchTestUnavailableBadge: '暂不可用',
      batchTestUnavailableHint: '上游/中转临时问题（没号、网关 502/503、服务繁忙、模型已下线无渠道），不是该账号的凭证错误。',
      batchTestUnavailableSummary: '{count} 个暂不可用',
      batchTestUnavailableNote: '这些是上游/中转的临时问题（中转没号、网关 502/503、模型下线无渠道），账号凭证未被证伪。默认保持启用，稍后或上游恢复后可重测。',
      batchTestKeepUnavailableEnabled: '保持启用（推荐）',
      batchTestDisableUnavailable: '仍要禁用这些账号',
      findDuplicates: '查找重复账号',
      findDuplicatesTitle: '查找重复账号',
      findDuplicatesNone: '在选中的 {count} 个账号里没有发现重复（按凭证身份、同平台同类型比对）。',
      findDuplicatesSummary: '发现 {groups} 组重复，共 {dupes} 个多余账号。每组建议保留最早创建的一个。',
      findDuplicatesHint: '判重按凭证身份比对（同平台、同类型才比），与账号名称无关。请确认每组保留哪个、删除哪些——勾选即为待删除，不会自动删除。',
      findDuplicatesGroup: '第 {index} 组 · {count} 个',
      findDuplicatesSuggestKeep: '建议保留',
      findDuplicatesCreatedAt: '创建于',
      findDuplicatesLastUsed: '最近使用',
      findDuplicatesWillKeep: '保留',
      findDuplicatesWillDelete: '待删除',
      findDuplicatesDeleteSelected: '删除选中的 {count} 个',
      findDuplicatesDeleteOk: '已删除 {count} 个重复账号。',
      findDuplicatesDeletePartial: '删除完成：成功 {ok} 个，失败 {failed} 个。',
      findDuplicatesAllSelectedWarning: '至少有一组的账号被全部勾选删除。每组请至少保留一个，避免整组清空。',
      findDuplicatesLoadFailed: '查找重复账号失败。',
      refresh: {
        errors: {
          REFRESH_TOKEN_REVOKED: 'Refresh Token 已失效或被吊销（账号可能已在网页端登出 / 改密 / 长期未用）。请删除该账号后重新走 OAuth 授权流程。',
          REFRESH_TOKEN_REUSED: 'Refresh Token 已被使用过（一次性 RT 已消耗）。请重新走 OAuth 授权流程获取新的 RT。',
          OAUTH_INVALID_CLIENT: 'OAuth 客户端配置错误。请检查 client_id / client_secret 是否被上游变更。',
          OAUTH_ACCESS_DENIED: '上游 OAuth 拒绝了访问。账号可能已被禁用或权限被回收，请重新授权。',
          OAUTH_NO_REFRESH_TOKEN: '该账号没有保存 refresh_token，无法自动刷新。请重新通过 OAuth 授权添加。',
          OAUTH_MISSING_PROJECT_ID: '该账号缺少 project_id，无法完成刷新。请重新走完整 OAuth 流程或手动补全 project_id。',
          OAUTH_UPSTREAM_UNAVAILABLE: '无法连接 OAuth 服务（网络中断 / 连接被对端关闭 / 超时）。请稍后重试，或检查代理 / 出口网络。',
          OAUTH_UPSTREAM_REJECTED: 'OAuth 上游拒绝了刷新请求，但未给出明确原因。请稍后重试，反复失败请重新授权。',
          OPENAI_OAUTH_PROXY_REQUIRED: '未配置可访问 OpenAI 的代理。请先为账号绑定可用代理，再重试刷新。',
          OPENAI_OAUTH_REQUEST_FAILED: 'OpenAI OAuth 请求失败（通常是代理 / 网络问题）。请稍后重试。',
          OPENAI_OAUTH_TOKEN_EXCHANGE_FAILED: 'OpenAI 拒绝了授权码兑换。请重新生成授权链接并完成授权。',
          OPENAI_OAUTH_TOKEN_REFRESH_FAILED: 'OpenAI 拒绝了 Token 刷新。请检查 Refresh Token 是否仍有效，或重新授权。',
          OPENAI_OAUTH_CLIENT_INIT_FAILED: 'OpenAI OAuth 客户端初始化失败。请检查代理配置后重试。',
          OAUTH_REFRESH_FAILED: 'OAuth 刷新失败，请稍后重试或重新授权。'
        }
      },
      ...
      dataImportDedupHint: '导入时会自动跳过与已有账号重复的项（按凭证身份、同平台同类型比对），不会重复创建。',
      dataImportSkippedTitle: '已跳过 {count} 个重复账号',
      dataImportSuccess: '导入完成：账号 {account_created}，跳过 {account_skipped}，失败 {account_failed}',
      ...
      crs: {
        errors: {
          CRS_INVALID_BASE_URL: 'CRS 服务地址无效。请检查 URL 格式（应以 https:// 开头）。',
          CRS_MISSING_CREDENTIALS: 'CRS 用户名或密码不能为空。',
          CRS_AUTH_FAILED: 'CRS 登录失败。请确认用户名 / 密码正确，且账号有效。',
          CRS_UPSTREAM_UNAVAILABLE: '无法连接 CRS 服务（网络中断 / 超时 / DNS 失败）。请稍后重试，或检查代理 / 出口网络。',
          CRS_EXPORT_FAILED: 'CRS 导出账号失败。请确认 CRS 版本 ≥ v1.1.240，且当前用户有 admin 权限。',
          CRS_SYNC_FAILED: 'CRS 同步失败。请稍后重试。'
        }
      },
      editAccount: '编辑账号',
      deleteAccount: '删除账号',
      ...
      bulkActions: {
        ...
        refreshFailureGroupReauth: '{count} 个需要重新授权',
        refreshFailureGroupTransient: '{count} 个网络/上游瞬时故障可稍后重试',
        refreshFailureGroupOther: '{count} 个其他错误'
      },
      bulkEdit: {
        title: '批量编辑账号',
  ...
  onboarding: {
    ...
    admin: {
      welcome: {
        title: '👋 欢迎使用 ANT-Sub2API-Local',
        description:
          '<div style="line-height: 1.8;"><p style="margin-bottom: 16px;">ANT-Sub2API-Local 是一个强大的 AI 服务中转平台，让您轻松管理和分发 AI 服务。</p><p style="margin-bottom: 12px;"><b>🎯 核心功能：</b></p><ul style="margin-left: 20px; margin-bottom: 16px;"><li>📦 <b>分组管理</b> - 创建不同的服务套餐（VIP、免费试用等）</li><li>🔗 <b>账号池</b> - 连接多个上游 AI 服务商账号</li><li>🔑 <b>密钥分发</b> - 为用户生成独立的 API Key</li><li>💰 <b>计费管理</b> - 灵活的费率和配额控制</li></ul><p style="color: #c15f3c; font-weight: 600;">接下来，我们将用 3 分钟带您完成首次配置 →</p></div>',
        nextBtn: '开始配置 🚀',
        prevBtn: '跳过'
      },
      groupManage: {
        title: '📦 第一步：分组管理',
        description:
          '<div style="line-height: 1.7;"><p style="margin-bottom: 12px;"><b>什么是分组？</b></p><p style="margin-bottom: 12px;">分组是 ANT-Sub2API-Local 的核心概念，它就像一个"服务套餐"：</p><ul style="margin-left: 20px; margin-bottom: 12px; font-size: 13px;"><li>🎯 每个分组可以包含多个上游账号</li><li>💰 每个分组有独立的计费倍率</li><li>👥 可以设置为公开或专属分组</li></ul><p style="margin-top: 12px; padding: 8px 12px; background: #f6e5dc; border-left: 3px solid #c15f3c; border-radius: 4px; font-size: 13px;"><b>💡 示例：</b>您可以创建"VIP专线"（高倍率）和"免费试用"（低倍率）两个分组</p><p style="margin-top: 16px; color: #c15f3c; font-weight: 600;">👉 点击左侧的"分组管理"开始</p></div>'
      },
      createGroup: {
        title: '➕ 创建新分组',
        description:
    ...
    user: {
      welcome: {
        title: '👋 欢迎使用 ANT-Sub2API-Local',
        description:
          '<div style="line-height: 1.8;"><p style="margin-bottom: 16px;">您好！欢迎来到 ANT-Sub2API-Local AI 服务平台。</p><p style="margin-bottom: 12px;"><b>🎯 快速开始：</b></p><ul style="margin-left: 20px; margin-bottom: 16px;"><li>🔑 创建 API 密钥</li><li>📋 复制密钥到您的应用</li><li>🚀 开始使用 AI 服务</li></ul><p style="color: #c15f3c; font-weight: 600;">只需 1 分钟，让我们开始吧 →</p></div>',
        nextBtn: '开始 🚀',
        prevBtn: '跳过'
      },
      keyManage: {
