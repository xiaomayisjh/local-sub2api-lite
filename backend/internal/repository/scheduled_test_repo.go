...
func NewScheduledTestPlanRepository(db *sql.DB) service.ScheduledTestPlanRepository {
	return &scheduledTestPlanRepository{db: SQLExecutorFromDB(db)}
}
...
func NewScheduledTestResultRepository(db *sql.DB) service.ScheduledTestResultRepository {
	return &scheduledTestResultRepository{db: SQLExecutorFromDB(db)}
}
...
func scanSinglePlan(rows *sql.Rows) (*service.ScheduledTestPlan, error) {
	if !rows.Next() {
		if err := rows.Err(); err != nil {
			return nil, err
		}
